...
const ParticleTextEffect: React.FC<ParticleTextEffectProps> = ({
    ...
    move(interactionRadius: number, hasPointer: boolean) {
      ...
      if (hasPointer && pointerRef.current.x !== undefined && pointerRef.current.y !== undefined) {
        ...
        if (dist < interactionRadius && dist > 0) {
          const force = Math.min(this.f, (interactionRadius - dist) / dist * 3);
  ...
  const write = () => {
    ...
    let fontSize = Math.min(100, canvas.height * 0.8 || 100);
    ctx.font = `900 ${fontSize}px Inter, sans-serif`;
    let textWidth = ctx.measureText(textBox.str).width;

    if (textWidth > canvas.width * 0.9 && canvas.width > 0) {
      fontSize = Math.floor(fontSize * ((canvas.width * 0.9) / textWidth));
      ctx.font = `900 ${fontSize}px Inter, sans-serif`;
      textWidth = ctx.measureText(textBox.str).width;
    }
    ...
    textBox.w = Math.round(textWidth);
